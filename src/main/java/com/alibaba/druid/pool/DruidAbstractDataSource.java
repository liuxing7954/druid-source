...
public abstract class DruidAbstractDataSource extends WrapperAdapter implements DruidAbstractDataSourceMBean, DataSource, DataSourceProxy, Serializable {
    ...
    // 如果存在连接泄漏,即有连接未关闭, 启动这个配置,会在超时后自动关闭, 影响性能
    ...
    // todo: 这个属性有点神奇, 很多地方都见到了.但还不知道是干嘛的
    ...
    /**
     * 校验conn可用性,根据配置druid.mysql.usePingMethod
     * 选择使用jdbc中自带的检测方法
     * @see com.mysql.cj.jdbc.ConnectionImpl#pingInternal(boolean, int)
     * 或者执行validationQuery
     *
     * 如果校验器validConnectionChecker 为null ,直接执行validationQuery还校验
     */
     ...
     * 检测conn还是否有效
     * 会再某种特殊情况
     * 即 该库支持校验器的时候,并且两次XX时间超过timeBetweenEvictionRunsMillis , 会进行抛弃连接处理
     * 目前一直没进入这个分支, 无法研究,估计是为高版本适配的
     * 该方法被使用的三个地方
     * 借出,还连接,或者借出时有一个空闲时间的判断
     * 三处地方都是可以配置的开关
     */
    protected boolean testConnectionInternal(DruidConnectionHolder holder, Connection conn) {
        ...
        try {
            if (validConnectionChecker != null) {
                // 根据druid.mysql.usePingMethod配置, 选择走自带jdbc里的ping方法.还是validateQuery
                ...
                if (valid && isMySql) {
                    // 当前驱动版本6.0的情况下, 该值一直是-1
                    long lastPacketReceivedTimeMs = MySqlUtils.getLastPacketReceivedTimeMs(conn);
                    if (lastPacketReceivedTimeMs > 0) {
                        //
                        long mysqlIdleMillis = currentTimeMillis - lastPacketReceivedTimeMs;
                        // 如果当前时间距离上一个时间,超过timeBetweenEvictionRunsMillis,则discardConnection连接
    ...
    protected volatile long connectionIdSeed  = 10_000L;
    protected volatile long statementIdSeed   = 20_000L;
    protected volatile long resultSetIdSeed   = 50_000L;
    protected volatile long transactionIdSeed = 60_000L;
    protected volatile long metaDataIdSeed    = 80_000L;
    ...
    /**
     * 这里有一段比较绕的策略链模式
     * 如果数据源有配置过滤器,那就会用这个模式,吧每个过滤器串成链子, 然后链子上每个过滤器回调
     * @see com.alibaba.druid.filter.Filter#connection_connect(FilterChain, Properties)
     * 除了stat过滤器,其他的好像就基本继承了一个过滤器适配器类,改变了一些回调的函数调用.更为简单继承
     *
     * 过滤器会回调过滤链的connection_connect方法,想获取创建好的conn来用,但是过滤器内部从不真正创建连接.
     * 所以最后创建conn的逻辑还是在过滤链中,当重复调用达到链子尾部,链子会去创建conn并返回.然后再回头经过一个个过滤器, 设置了一堆属性或者回调了一些逻辑.
     */
    ...
    public PhysicalConnectionInfo createPhysicalConnection() throws SQLException {
        ...
        try {
            ...
            // 初始化连接的属性  可能执行初始化sql  可能获取数据库配置信息
    ...
    /**
     * 初始化赋值一些连接的属性,然后执行初始化sql,这个是配置的, 没配过.
     * @param conn
     * @param variables 根据initVariants配置来选择是否获取数据库属性到 连接封装类里
     * @param globalVariables 同上 initGlobalVariants
     * @throws SQLException
     */
    ...
    public void discardConnection(DruidConnectionHolder holder) {
        System.out.println("父类 的 discard");
